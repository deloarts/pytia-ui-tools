...
from pytia_ui_tools.models.files import FileUtilityDeleteModel
from pytia_ui_tools.models.files import FileUtilityMoveModel
...
class FileUtility:
    ...
    def move_item(self, item: FileUtilityMoveModel) -> None:
        ...
        if not item.moved and not item.skipped:
            while os.path.exists(item.target) and not os.path.isdir(item.target):
